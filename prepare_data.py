...
from data_loader import InputFeatures_train
...
def _make_feature(id_, sents, ws, eos):
    if all(w == 0 for w in ws[1:]): return None
    input_ids = [i for s in sents for i in s+[eos]][:-1]
    lm_labels, weights, token_type_ids = [], [], [] # this becomes round ids 
    ...
    if len(input_ids) == 0:
        import pdb
        pdb.set_trace()
    feature = InputFeatures_train(id_, input_ids, position_ids, token_type_ids, lm_labels, weights)
    return feature
...
def main(args):
    ...
    attrs = []
    if args.reverse: attrs.append('reverse')
    if args.two_turn: attrs.append('2turn')
    if attrs:
        db_path = (f'{args.corpus[:-4]}.{args.max_seq_len}len.{".".join(attrs)}.db/db')
    else:
    ...
    db = shelve.open(db_path, 'n')                   
                   
    chunk, n_chunk, n_example = [], 0, 0
    for line in tqdm(reader, total=_get_file_len(args.corpus)):
        try:
            if len(chunk) >= args.chunk_size:
                # save and renew chunk
                db[f'chunk_{n_chunk}'] = gzip.compress(json.dumps(chunk[:args.chunk_size]).encode('utf-8'))
            ...
            features = _make_features(n_example, weights, inputs, toker, args.max_seq_len)
    ...
    meta = {'n_example': n_example, 'chunk_size': args.chunk_size,
          'max_seq_len': args.max_seq_len, 'reverse': args.reverse, 'two_turn': args.two_turn}
    json.dump(meta, open(join(dirname(db_path), 'meta.json'), 'w'), indent=4)
    torch.save(toker, join(dirname(db_path), 'tokenizer.pt'))


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--corpus', required=True, help='file name of training corpus (should be .tsv)')
    parser.add_argument('--chunk_size', type=int, default=65536, help='num of data examples in a storing chunk')
    parser.add_argument('--max_seq_len', type=int, default=128, help='discard data longer than this')
    parser.add_argument('--reverse', action='store_true', help='reverse the src tgt')
    parser.add_argument('--two_turn', action='store_true', help='take only the first 2 turns')
